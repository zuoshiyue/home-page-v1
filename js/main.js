...
fetch('https://restapi.amap.com/v3/weather/weatherInfo?city=110000&extensions=base&key=34752fd7db29f988eca725b5af12102d')
    .then(response => response.json())
    .then(data => {
        console.log(`天气获取API `+ JSON.stringify(data.lives))
        console.log(`天气状态 `+ JSON.stringify(data.lives[0].weather))
        console.log(`天气城市 `+ JSON.stringify(data.lives[0].city))
        console.log(`天气温度 `+ JSON.stringify(data.lives[0].temperature))
        $('#wea_text').html(data.lives[0].weather)
        $('#city_text').html(data.lives[0].city)
        $('#tem_day').html(data.lives[0].temperature)
